...
if __name__ == '__main__':
	...
	for iproc in range(nnstart,nn):
		...
		normalized_features[:,3] = (data[:, 5] + twopi) / fourpi
		normalized_features[:,4] = (data[:, 6] + twopi) / fourpi
		normalized_features[:,5] = (data[:, 7] + twopi) / fourpi
	...
	np.savetxt(os.path.join(feature_dir, 'normalized_features{}__{}_{}_6.txt'.format(nnstart, total_runs, expected_size)), all_feat_normed)
	np.savetxt(os.path.join(feature_dir, 'features{}__{}_{}_6.txt'.format(nnstart, total_runs, expected_size)), all_feat)
